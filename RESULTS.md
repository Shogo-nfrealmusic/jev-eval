...
Every number is copied from `results/summary.json` / `results/summary-sonnet60.json` / `results/summary-sonnet.json` (apart from rounding for display, nothing was recomputed by hand).
...
| Claude run (60 × 3) | 2026-09-18 11:51:37 – 12:04:09 UTC (20:51 – 21:04 JST) / run `2026-09-18T11-51-37-247Z` |
| Pilot Claude run (20 × 1, superseded) | 2026-09-18 11:31:50 UTC (20:31 JST) / run `2026-09-18T11-31-50-447Z` |
| Model under test | `typesafe-ai/jev` |
| Comparison model (main run) | `openai/gpt-4o-mini` |
| Comparison model (Claude run) | `anthropic/claude-sonnet-4.5` |
...
| Claude run size | 60 cases × 3 rounds = 180 calls per model (+ 1 discarded warm-up), Jev re-measured alongside |
| Errors | **1 of 380 Jev calls; 0 of 380 LLM calls.** In the Claude run, one Jev call (h11, round 3) got no response and hit the client timeout after 300 s (`GatewayTimeoutError 408`). Not retried (zero-retry rule); it is excluded from that run's Jev accuracy (n = 179) and latency |
...
Claude Sonnet 4.5 never returned exactly 0.5 (91.7% under both thresholds, 60 × 3 run).
...
## 4. How the multipliers change with the comparison model (Claude Sonnet 4.5: 60 cases × 3 rounds)

gpt-4o-mini is among the cheapest and fastest models. To see whether the multipliers depend on the comparison model, we ran the larger `anthropic/claude-sonnet-4.5` on the full set under exactly the same conditions (serial, 1 discarded warm-up, zero retries, Jev and Sonnet called alternately within the same run).

| 60 cases × 3 rounds | Jev (same run) | claude-sonnet-4.5 |
|---|---|---|
| Category accuracy (exact) | 95.5% (171/179) | 94.4% (170/180) |
| Category accuracy (incl. acceptable alternatives) | 98.3% | 98.3% |
| 　Straightforward / hard | 100% / 86.4% | 100% / 83.3% |
| Urgency agreement (≥0.5 / >0.5) | 96.6% / 96.6% | 91.7% / 91.7% |
| needs_human agreement | 82.7% | 85.0% |
| needs_human misses | 20 / 68 | 18 / 69 |
| Latency p50 / p95 | **382 / 508 ms** | 1,910 / 2,162 ms |
| Cost per 1,000 messages (billed) | **$0.032** | $3.41 |
| Avg tokens (input / output) | 764 / 119 | 1,025 / 22 |
| Errors | 1 (timeout, see §1) | 0 |

**Multipliers relative to Jev (all measured; each uses Jev from the same run)**

| Compared with | Speed (p50 ratio) | Cost | Category accuracy (exact) |
|---|---|---|---|
| gpt-4o-mini (60 × 3) | 3.2× faster | 3.8× cheaper | Jev 96.1% vs 93.9% |
| claude-sonnet-4.5 (60 × 3) | 5.0× faster | 106× cheaper | Jev 95.5% vs 94.4% |
| TypeSafe's published figures | 193.6× faster | 444.6× cheaper | — |

**Takeaway**: the speed multiplier stayed around 3–5× regardless of the comparison model, but the cost multiplier **ranged from 3.8× to 106× depending on what Jev is compared with**. Category accuracy was within about 2 points across all three models.
...
**Jev re-measured in the Claude run** gave nearly the same results as in the main run (category 95.5% vs 96.1%, p50 382 vs 379 ms, same $0.032 per 1,000), so the two runs are comparable.

**Sonnet's category errors**: h15 → late_arrival (3/3, same as Jev and gpt-4o-mini), h16 → group_size (3/3), h19 (wedding shoot in Hokkaido) → other (3/3), h06 → reschedule (1/3). It correctly answered other for h07 ("what time tomorrow?"), which Jev got wrong.

A 20-case × 1-round pilot with Sonnet (run `2026-09-18T11-31-50-447Z`, `results/summary-sonnet.json`) gave the same picture (4.6× faster, 106× cheaper, 90% vs 90%); it is superseded by the full run above.
...
These are cases that need a human but were predicted false. **This is Jev's weakest area**: 21/69 misses (gpt-4o-mini: 16/69, Claude Sonnet 4.5: 18/69). **But the LLMs miss largely the same cases**: Sonnet also missed h01, h08, h15, h17, h18 and h20 in all 3 rounds.
Jev missed the following 7 cases the same way in all 3 rounds (round-by-round values are in `results/per-case.json`).

| id | Message | Gold | Jev P(needs_human) | Jev category (prob.) | gpt-4o-mini | Sonnet 4.5 |
|---|---|---|---|---|---|---|
| **h01** | 台風で飛行機が飛ばないので、明日の撮影を土曜に変えたいです。 (Our flight is grounded by the typhoon, so we'd like to move tomorrow's shoot to Saturday.) | true | **0.40 – 0.43** | reschedule (1.00) | true (3/3 correct) | 3/3 missed |
| **h08** | Please unsubscribe me from this mailing list. | true | **0.25 – 0.30** | other (0.99) | false (3/3 missed) | 3/3 missed |
| h15 | We might be a bit late tomorrow morning … should we move it later? | true | 0.29 – 0.30 | late_arrival (0.83–0.87) | 1/3 missed | 3/3 missed |
| h17 | 촬영장소가 비가 오면 실내로 바뀌나요? 그럼 가격도 달라지나요? (If it rains, does the shoot move indoors? Does the price change then?) | true | 0.38 – 0.39 | weather (0.99) | 3/3 missed | 3/3 missed |
| h18 | 請問可以改期嗎 (Can I reschedule?) | true | 0.28 – 0.33 | reschedule (1.00) | 3/3 missed | 3/3 missed |
| h19 | I booked with you guys last year and loved it! Do you do wedding shoots in Hokkaido?? 😍📸 | true | 0.26 | location (0.97–0.98) | 3/3 missed | true (3/3 correct) |
| h20 | 취소해주세요. (Please cancel.) | true | 0.47 – 0.49 | cancel (1.00) | true (3/3 correct) | 3/3 missed |
...
- Claude Sonnet 4.5: reschedule, but needs_human = false in all 3 rounds (missed, same as Jev)
...
- Claude Sonnet 4.5: other, needs_human = false in all 3 rounds (missed, same as Jev)
...
**What the 7 cases have in common**: the category is correct with high probability, but needs_human sits just below 0.5. Because all three models miss most of these, part of the problem is likely the rule itself (or our labels): "needs a human" when information is missing or the request is out of scope is hard to infer from the question wording alone.
...
Jev in the Claude run (60 × 3) showed the same pattern: 88.3% (158 calls) auto-processed with category 100% correct, and 17 of them (h01, h08, h17, h18, h19, h20) were needs_human misses.
...
- Jev in the Claude run (60 × 3) had the same shape: baseline 158 auto-processed with 17 pass-throughs; X = 0.25 gives 0 pass-throughs (35 auto-processed, 19.6%), X = 0.30 gives 6, X = 0.50 still leaves 17

---

## 9. Category accuracy by gold category (exact match, all rounds pooled; Jev and gpt-4o-mini from the main run, Sonnet from the Claude run)

| Gold category | Calls | Jev | gpt-4o-mini | Sonnet 4.5 |
|---|---|---|---|---|
| reschedule | 27 | 88.9% | 88.9% | 88.9% |
| cancel | 18 | 100% | 100% | 100% |
| refund | 18 | 100% | 100% | 100% |
| location | 18 | 100% | 100% | 83.3% |
| pricing | 18 | 100% | 88.9% | 83.3% |
| group_size | 18 | 94.4% | 100% | 94.4% |
| late_arrival | 21 | 100% | 100% | 100% |
| weather | 24 | 100% | **75.0%** | 100% |
| other | 18 | **83.3%** | 100% | 100% |
...
- Sonnet 4.5 went the other way on h19 (wedding shoot in Hokkaido → other) and pulled h16 toward group_size
...
**Won**
- Speed: 3.2× faster than gpt-4o-mini and 5.0× faster than Sonnet 4.5 at p50. p95 was a steady 481 ms
...
**Lost**
- **Most needs_human misses of the three**: 21/69 (gpt-4o-mini 16/69, Sonnet 4.5 18/69). It missed h01, the example from the brief (so did Sonnet)
...
- **One call hung**: 1 of 380 Jev calls returned nothing and timed out after 300 s (0 of 380 LLM calls failed). Small numbers, but a production integration needs its own timeout and fallback
...
- **The multipliers depend on the comparison model**: the cost multiplier ranged from 3.8× to 106× between gpt-4o-mini and Sonnet 4.5. Each comparison model was measured in its own run (with Jev re-measured alongside), not all three in one run
- **n = 60**: one case changes accuracy by 1.7 points. The accuracy gaps between models (e.g. 96.1% vs 93.9%) cannot be called statistically meaningful
...
- Scoring: `src/score.ts` → `results/summary.json`, `results/per-case.json` (the Claude 60 × 3 run's files carry a `-sonnet60` suffix; the 20-case pilot's carry `-sonnet`)
